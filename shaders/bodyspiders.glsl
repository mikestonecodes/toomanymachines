...
void spider_emissive(vec2 p, Body b) {
	...
	float beat = 0.85 + 0.35 * sin(pc.time * 2.8 + float(v_id) * 0.7); // eyes PULSE like a heartbeat — alive, never near-dark
	for (float s = -1.0; s <= 1.0; s += 2.0) {
		add += gEye * 1.5 * beat * soft(length(p - vec2(r * 0.58, s * r * 0.28)) - r * 0.10);
	}
}
...
void skitter_emissive(vec2 p, Body b) {
	float r = b.radius;
	float beat = 0.85 + 0.35 * sin(pc.time * 3.4 + float(v_id) * 0.9); // pulsing eye — alive, never near-dark
	add += gEye * 1.6 * beat * soft(length(p - vec2(r * 0.95, 0.0)) - r * 0.13);
}
...
void brute_emissive(vec2 p, Body b) {
	float r = b.radius;
	float pulse = 0.9 + 0.25 * sin(pc.time * 3.0 + float(v_id)); // core vents: two hot slits, pulsing
	for (float s = -1.0; s <= 1.0; s += 2.0) {
		add += gEye * 1.35 * pulse * soft(sd_box(p - vec2(-r * 0.30, s * r * 0.14), vec2(r * 0.19, r * 0.034)));
	}
	for (float s = -1.0; s <= 1.0; s += 2.0) {
