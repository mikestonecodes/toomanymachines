...
//   • the game (body.frag) — draws every body live, and for the alive horde SAMPLES the baked
...
// the gait odometer) live in body.frag: they can't be baked, and the baker has no v_id.
#ifndef BODYKIT_GLSL
#define BODYKIT_GLSL
...
// live in body.frag (or, for the atlas path, re-added procedurally after the fetch). Baking these
...
void spider_chassis(vec2 p, Body b, float gt) {
	...
	// paired sensor EYE SOCKETS on the front hull corners (the glow is emissive → body.frag)
