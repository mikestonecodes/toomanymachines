...
// [kind × gait-frame] tiles. The runtime horde (body.frag: enemy_atlas) then FETCHES a tile
...
// eyes, exhaust embers, battle damage) is NOT baked — body.frag re-adds it live.
//
// Output per texel: RGB = diffuse base, A = coverage. gl_FragCoord picks the tile + the in-tile
// body-frame position, the exact inverse of body.frag's p→uv mapping (ATLAS_EXTK · ATLAS_RAD).
...
void main() {
	...
	// the horde's paint (mirrors body.frag main()'s enemy branch) — the DIFFUSE values the
